...
# -*- coding: utf-8 -*-
"""
通用论文 JSON → 规范实验脉络数据集转换程序
Universal converter: raw PDF-parsed JSON → structured experimental trajectory dataset

输入: 由PDF解析器生成的原始JSON（含 pdf_info / preproc_blocks 结构）
输出: 参照附件1格式的规范数据集JSON

用法:
    直接修改下方 INPUT_JSON 变量为目标文件名，然后运行：
    python pdf_json_to_dataset.py
"""
...
INPUT_JSON = "D:/work/MinerU/JSON格式/姬婷-比赛文献1-酚醛树脂与聚丙烯酸丁酯嵌段苯乙烯共聚物和氧化铝纳米颗粒提高热稳定性以及韧性同时降低初始固化温度__20260522010646.json"          # ← 修改为你的 JSON 文件名（支持相对/绝对路径）
OUTPUT_JSON = "D:/work/MinerU/JSON格式/提取后/姬婷-1.json"                   # ← 留空则自动生成（与输入同名，后缀改为 _dataset.json）
